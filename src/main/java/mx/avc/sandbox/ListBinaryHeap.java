...
import static mx.avc.sandbox.BaseListBinaryHeap.siftUp;
 ...
 * @author alexv
 */
public class ListBinaryHeap<T> implements Heap<T> {
     ...
     */
    public ListBinaryHeap(Comparator<T> c) {
        heap = new ArrayList<>();
        comparator = c;
    }
     ...
     */
    ListBinaryHeap(ArrayList<T> h, Comparator<T> c) {
        heap = h;
        comparator = c;
    }
     ...
     */
    public ListBinaryHeap(Comparator<T> c, Collection<? extends T> initial) {
        this(c);
     ...
     */
    @SuppressWarnings("unchecked")
    public ListBinaryHeap(Collection<? extends T> initial) {
     ...
     */
    @Override
    public ListBinaryHeap<T> pop(int count) {
        if(count < 0) {
            throw new IllegalArgumentException();
        }

        if(count == 0 || heap.isEmpty()) {
            return new ListBinaryHeap<>(comparator);
        }
        ...
        if(count >= size) {
            ...
            heap.clear();
            return new ListBinaryHeap<>(head, comparator);
        }
